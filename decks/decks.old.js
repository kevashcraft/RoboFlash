...
const fullDecksBucket = 'rlf-full-decks'
...
async function getWords(deckDataSpreadsheet, deckName) {
    const deckSheet = deckDataSpreadsheet.sheetsByTitle[deckName]

    await deckSheet.loadCells(`A1:B${MAX_WORD_ROWS}`);

    const cards = []
    
    for (let index=0; index<MAX_WORD_ROWS; rowIdx++) {
        const words = {}
        languages.forEach((language, idx) => {
            words[language.slug] = deckSheet.getCell(rowIdx, idx).value
        })

        cards.push({ index: rowIdx, words })
    }
